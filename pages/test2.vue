<template>
  ...
  d-flex justify-center" style="margin-bottom: 65px;">
         ...
         <v-btn color="#F5F5DC" style="color:#000;" @click="loadMore()" v-if="showButton == true" depressed>More</v-btn>
       </div>
     </v-col>
   </v-row>
   </template>
...
//get from indexeddb
    ...
    loadMore() {
      ...
      let currentPosition = window.scrollY;
        let targetPosition = currentPosition + 300;
        let step = 30;

function scrollSmoothly() {
  let newPosition = window.scrollY + step;

  if (step > 0 && newPosition >= targetPosition ||
      step < 0 && newPosition <= targetPosition) {
...
let scrollInterval = setInterval(scrollSmoothly, 15);
